            ...
            @auth
                <div class="hidden lg:flex gap-x-6">
                    @if (auth()->user()->type != 'admin')
                        <a href="{{ 
                            auth()->user()->type == 'agent' ? route('agents.dashboard') : 
                            (auth()->user()->type == 'dealer' ? route('dealers.dashboard') : 
                            route('customers.dashboard')) 
                        }}">
                            <p class="text-brand-dark-green text-sm">Dashboard</p>
                        </a>
                    @else
                        <div class="hidden lg:flex gap-x-6">
                    ...
                    @if (auth()->user()->type == 'customer')
                        <a href="/start-application">
                            <p class="text-brand-dark-green text-sm">Start Application</p>
                        </a>
                    @endif
                </div>
            @else
