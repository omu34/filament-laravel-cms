    ...
    <div x-data="{ loginModalOpen: false }" @open-modal.window="loginModalOpen = true">
        <div x-data="{}" x-ref="search" x-show="loginModalOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex min-h-full items-center justify-center p-4 text-center">
                ...
                <div class="relative bg-white border-solid border-2 border-red-950 rounded-lg mx-auto max-w-3xl pt-4 pb-10 px-4" x-data="{}" x-cloak x-show="loginModalOpen"
                x-transition:enter="transition ease-out duration-300 transform"
                    ...
                    <div class="w-full flex flex-col justify-start items-start gap-4 px-6 mt-2">
                        <div class="w-full flex justify-start items-start">
                            <div class="grid gap-4 justify-items-start">
                                <h5 class="text-white">Track Application</h5>
                                <form wire:submit="login" class="w-full grid gap-y-4">
                                    <div class="grid grid-cols-1 gap-2 custom-select">
                                        {{-- <label class="text-left text-red-950" for="email">
                                            Email address
                                            <span class="text-required-red">*</span>
                                        </label> --}}
                                        <input required placeholder="Email address" class="border-solid border-2 border-red-950" type="email" wire:model="form.email" name="email" id="email" class="w-full">
                                        <x-input-error :messages="$errors->get('form.email')" class="mt-2" />
                                    </div>
                                    <div class="grid grid-cols-1 gap-2 custom-select">
                                        {{-- <label class="text-left text-red-950" for="password">
                                            Password
                                            <span class="text-required-red">*</span>
                                        </label> --}}
                                        <input required class="border-solid border-2 border-red-950" placeholder="Password" type="password" wire:model="form.password" name="password" id="password" class="w-full">
                                        <x-input-error :messages="$errors->get('form.password')" class="mt-2" />
                                    </div>
                                    <div class="mt-2 w-full flex">
                                        <input required class="border-solid border-2 border-red-950" wire:model="form.remember" id="remember" type="checkbox" class="w-5 h-5 mr-2 rounded-md checked:bg-red-950" name="remember">
                                        <label for="remember" class="text-red-950">
                                            Remember me
                                        </label>
                                    </div>
                                    <div class="w-full flex">
                                        <button type="submit" wire:loading.class="opacity-50 cursor-not-allowed" class="w-full px-4 py-2 flex items-center justify-center rounded-lg bg-red-900 hover:bg-red-700 text-white border-2 shadow-md text-sm">
                                            Login
                                    ...
                                    <div class="w-full flex gap-4 justify-between">
                                        <a href="#">
                                            <span class="text-red-950 text-sm">No account?</span>
                                        </a>
                                        @if (Route::has('password.request'))
                                            <a href="{{ route('password.request') }}" wire:navigate>
                                                <span class="text-red-950 text-sm">Forgot Password?</span>
                                            </a>
                                        @endif
                                    </div>
                                    <a href="/application" class="w-full flex items-center justify-center px-4 py-2 text-sm rounded-lg bg-white border-red-950  hover:bg-red-700 text-red-950 border-2 shadow-md">
        ...
        {{-- Trigger button --}}
        <div class="px-4 py-2 w-full hidden sm:flex items-center justify-center rounded-lg bg-red-900 hover:opacity-90 text-white border-2 border-white shadow-md cursor-default" x-on:click="loginModalOpen = !loginModalOpen">
            <button type="button" class="text-sm">
                Track Application
            </button>
            <svg class="ml-1" width="25" height="25" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path opacity="0.4" d="M16.5 9.5C16.5 13.366 13.366 16.5 9.5 16.5C5.63401 16.5 2.5 13.366 2.5 9.5C2.5 5.63401 5.63401 2.5 9.5 2.5C13.366 2.5 16.5 5.63401 16.5 9.5Z" fill="white" stroke="white"/>
                <path d="M8.55477 12.71C8.41227 12.71 8.26977 12.6575 8.15727 12.545C7.93977 12.3275 7.93977 11.9675 8.15727 11.75L10.4073 9.5L8.15727 7.25C7.93977 7.0325 7.93977 6.6725 8.15727 6.455C8.37477 6.2375 8.73477 6.2375 8.95227 6.455L11.5998 9.1025C11.8173 9.32 11.8173 9.68 11.5998 9.8975L8.95227 12.545C8.83977 12.6575 8.69727 12.71 8.55477 12.71Z" fill="white"/>
            </svg>
        </div>
        <div class="flex sm:hidden cursor-default" x-on:click="loginModalOpen = !loginModalOpen">
            <h5 class="text-red-950 text-bold">Track Application</h5>
        </div>
    </div>
</div>
