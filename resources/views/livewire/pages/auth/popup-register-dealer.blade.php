    ...
    <div x-data="{ registerDealerModalOpen: false }">
        <div x-data="{}" x-ref="search" x-show="registerDealerModalOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex min-h-full items-center justify-center p-4 text-center">
                ...
                <div class="relative bg-white border-solid border-2 border-red-950 rounded-lg mx-auto max-w-3xl pt-4 pb-10 px-4" x-data="{}" x-cloak x-show="registerDealerModalOpen"
                x-transition:enter="transition ease-out duration-300 transform"
                    ...
                    <div class="w-full flex flex-col justify-start items-start gap-4 px-5">
                        <div class="w-full flex justify-start items-start gap-10">
                            <div class="grid gap-8 justify-items-start">
                                <h4 class="text-red-950">Register as a Dealer</h4>
                                <form wire:submit.prevent="register" class="grid gap-y-8 w-full">
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-8 w-full">
                                        <div class="grid grid-cols-1 gap-2">
                                            <label class="text-left text-red-900" for="cname">Company Name<span class="text-required-red">*</span></label>
                                            <input class="border-solid border-2 border-red-950" type="text" name="cname" id="cname" wire:model="company_name">
                                            @error('company_name')
                                                <span class="text-required-red">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div class="grid grid-cols-1 gap-2 custom-select">
                                            <label class="text-left text-red-900" for="location">Physical Location<span class="text-required-red">*</span></label>
                                            <input class="border-solid border-2 border-red-950" type="text" name="location" id="location" wire:model="location">
                                            @error('location')
                                                <span class="text-required-red">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div class="grid grid-cols-1 gap-2 custom-select">
                                            <label class="text-left text-red-900" for="first_name">First name<span class="text-required-red">*</span></label>
                                            <input class="border-solid border-2 border-red-950" type="text" name="first_name" id="first_name" wire:model="first_name">
                                            @error('first_name')
                                                <span class="text-required-red">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div class="grid grid-cols-1 gap-2 custom-select">
                                            <label class="text-left text-red-900" for="last_name">Last name<span class="text-required-red">*</span></label>
                                            <input class="border-solid border-2 border-red-950" type="text" name="last_name" id="last_name" wire:model="last_name">
                                            @error('last_name')
                                                <span class="text-required-red">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div class="grid grid-cols-1 gap-2 custom-select">
                                            <label class="text-left text-red-900" for="e_add">Email address<span class="text-required-red">*</span></label>
                                            <input class="border-solid border-2 border-red-950" type="email" name="e_add" id="e_add" wire:model="email">
                                            @error('email')
                                                <span class="text-required-red">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div class="grid grid-cols-1 gap-2 custom-select">
                                            <label class="text-left text-red-900" for="pno">Phone number<span class="text-required-red">*</span></label>
                                            <input class="border-solid border-2 border-red-950" type="text" name="pno" id="pno" wire:model="phone">
                                            @error('phone')
                                                <span class="text-required-red">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div class="grid grid-cols-1 gap-2 custom-select">
                                            <label class="text-left text-red-900" for="dealer_pass">Password<span class="text-required-red">*</span></label>
                                            <input class="border-solid border-2 border-red-950" type="password" name="dealer_pass" id="dealer_pass" wire:model="password">
                                            @error('password')
                                                <span class="text-required-red">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div class="grid grid-cols-1 gap-2 custom-select">
                                            <label class="text-left text-red-900" for="c_dealer_pass">Confirm Password<span class="text-required-red">*</span></label>
                                            <input class="border-solid border-2 border-red-950" type="password" name="c_dealer_pass" id="c_dealer_pass" wire:model="confirm_password">
                                    ...
                                    <div class="mt-2 w-full flex justify-center">
                                        <div class="w-44 sm:w-1/2 px-4 py-2 flex items-center justify-center rounded-lg  bg-red-950 hover:bg-red-700 text-white border-2 shadow-md">
        ...
        <div class="w-full hidden lg:flex items-center cursor-default hover:opacity-80 text-nowrap" x-on:click="registerDealerModalOpen = !registerDealerModalOpen">
            <p class="text-red-800 text-sm">Register as Dealer</p>
            <svg class="ml-1" width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path opacity="0.4" d="M16 9.5C16 13.366 12.866 16.5 9 16.5C5.13401 16.5 2 13.366 2 9.5C2 5.63401 5.13401 2.5 9 2.5C12.866 2.5 16 5.63401 16 9.5Z" fill="#" stroke="#000000"/>
        ...
        <div class="flex lg:hidden cursor-default" x-on:click="registerDealerModalOpen = !registerDealerModalOpen">
            <h5 class="text-red-950 text-bold">Register as a Dealer</h5>
        </div>
    </div>
